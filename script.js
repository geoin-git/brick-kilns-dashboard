...
// Dynamically determine the data file path to work both locally and on GitHub Pages
// Uses relative path resolution that works regardless of hosting location
function getDataUrl() {
    ...
    try {
        const resolvedUrl = new URL(dataPath, baseUrl);
        return resolvedUrl.pathname; // Return just the pathname part
    } catch (e) {
        ...
        return pathname + dataPath;
    }
}

// Set DATA_URL dynamically - works both locally and on GitHub Pages
const DATA_URL = getDataUrl();

// Log the resolved data URL for debugging
console.log('Data file URL resolved to:', DATA_URL);
